...
class PatchTSTSelfSupervised(nn.Module):
    def __init__(self, input_length, patch_len, stride, d_model=128, n_heads=8, n_layers=3, mask_ratio=0.4):
        super().__init__()
    ...
    def forward(self, x):
        ...
        masked_patches = masked_patches.masked_fill(mask.unsqueeze(-1).expand_as(masked_patches), 0.0)
    ...
    def _create_patches(self, x):
        ...
        return x.unfold(dimension=-1, size=self.patch_len, step=self.stride)  # (B, V, N, P)
